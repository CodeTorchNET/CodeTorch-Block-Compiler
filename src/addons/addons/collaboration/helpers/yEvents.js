...
export function setupYEventsObserver() {
    ...
    constants.mutableRefs.yEvents.observe(event => {
        ...
        if (!constants.mutableRefs.vm || !constants.mutableRefs.vm.runtime) {
            CollaborationConsole.warn('Collab RX: VM or VM runtime not ready. Skipping event processing.');
            return;
        }
        ...
        if (!workspace && event.changes.added.some(item => item.content?.getContent()?.[0]?.event?.type)) {
            CollaborationConsole.warn('Collab RX: Workspace not found, cannot apply remote standard Blockly events.');
        }
        ...
        event.changes.added.forEach(item => {
            ...
            if (item.content && typeof item.content.getContent === 'function') {
                ...
                CollaborationConsole.warn('Collab RX (yEvents observer): Received unexpected item structure in yEvents change:', item);
            }
        });

        // If no event items were received, there's nothing to process.
        if (allReceivedEventItemsInBatch.length === 0) return;
        if (constants.debugging) CollaborationConsole.log(`Collab RX (yEvents observer): Received ${allReceivedEventItemsInBatch.length} remote event items.`);
        
        // Record all received YJS events
        allReceivedEventItemsInBatch.forEach(item => {
            recorder.recordYjsEvent('RECEIVE', 'yEvents', item);
        });
        ...
        if (!constants.mutableRefs.hasProcessedInitialBlockEvents) {
            ...
            if (lastSaveProjectIndexInBatch !== -1) {
                ...
                if (constants.debugging) {
                    CollaborationConsole.log(`Collab RX (yEvents observer): Initial event batch. Found 'savedProject'. Processing ${itemsToProcess.length} of ${allReceivedEventItemsInBatch.length} items from this batch, starting from the save event.`);
                }
            } else {
                if (constants.debugging) {
                    CollaborationConsole.log(`Collab RX (yEvents observer): Initial event batch. No 'savedProject' found in this specific batch. Processing all ${allReceivedEventItemsInBatch.length} items from this batch.`);
                }
            }
        ...
        if (constants.debugging && itemsToProcess !== allReceivedEventItemsInBatch) {
            CollaborationConsole.log(`Collab RX (yEvents observer): Filtered to process ${itemsToProcess.length} events.`);
        } else if (constants.debugging && constants.mutableRefs.hasProcessedInitialBlockEvents) {
            CollaborationConsole.log(`Collab RX (yEvents observer): Received ${itemsToProcess.length} remote event items to process (subsequent batch).`);
        }
...
export function processSpecificEvent(item) {
    if (constants.debugging) CollaborationConsole.log(`Collab RX: Processing event item:`, item);
    ...
    if (!item) {
        CollaborationConsole.warn('Collab RX: Received null/undefined event item. Skipping.');
        return;
    }
    ...
    if (eventType === 'var_create' || eventType === 'var_delete' || eventType === 'var_rename') {
        ...
        if (!currentWorkspace) {
            CollaborationConsole.warn(`Collab RX: Workspace not found, cannot apply variable event type "${eventType}". Skipping.`);
            return;
        }
        ...
        const getVmVariableByName = (variableName, contextTargetName, ambiguityFix) => {
            if (!(ambiguityFix && typeof ambiguityFix.varType !== "undefined" && typeof ambiguityFix.isCloud !== "undefined")) {
                CollaborationConsole.warn(`Collab RX: getVmVariableByName called without ambiguityFix, which is required for correct variable type resolution.`);
                return null;
            }
            ...
            if (vmTargetForCreate) {
                ...
                if (!existingVar) {
                    try {
                        ...
                        if (constants.debugging) CollaborationConsole.log(`Collab RX: Created variable "${newVarData.name}" on target ${vmTargetForCreate.getName()}, refreshing toolbox?`, shouldRefreshToolbox);
                    } catch (e) {
                        CollaborationConsole.error(`Collab RX: Error creating variable from Yjs for target ${vmTargetForCreate.getName()}:`, newVarData.name, e);
                    }
                } else if (constants.debugging) {
                    CollaborationConsole.log(`Collab RX: Variable "${newVarData.name}" already exists on target ${vmTargetForCreate.getName()}, skipping creation.`);
                }
            } else {
                CollaborationConsole.warn(`Collab RX [var_create]: Target sprite "${newVarData.targetName}" not found for local variable "${newVarData.name}" creation.`);
            }
        }
        // --- Handle Variable Renaming ---
            ...
            if (!renameData.ambiguityFix || typeof renameData.ambiguityFix.varType === "undefined" || typeof renameData.ambiguityFix.isCloud === "undefined" || typeof renameData.ambiguityFix.isLocal === "undefined") {
                CollaborationConsole.warn(`Collab RX: var_rename called without ambiguityFix, which is required for correct variable type resolution. Skipping.`);
                return;
            }
            ...
            if (variableToRename) {
                if (vmTargetForAction) {
                    ...
                    if (conflict) {
                        CollaborationConsole.warn(`Collab RX: Rename conflict for variable "${renameData.newName}" on target ${vmTargetForAction.getName()}. Skipping.`);
                        return;
                    }
                    ...
                    if (vmTargetForAction.variables[variableToRename.id] && vmTargetForAction.variables[variableToRename.id].name !== renameData.newName) {
                        try {
                            ...
                            if (constants.debugging) CollaborationConsole.log(`Collab RX: Renamed variable on ${vmTargetForAction.getName()}: "${renameData.oldName}" -> "${renameData.newName}"`);
                        } catch (e) {
                            CollaborationConsole.error(`Collab RX: Error renaming variable on ${vmTargetForAction.getName()}:`, e);
                        }
                    }
                        ...
                        try {
                            ...
                            if (blocklyVariable.type === "broadcast_msg") {
                                ...
                                if (variableToRename.value !== renameData.newName) {
                                    ...
                                    if (constants.debugging) CollaborationConsole.log(`Collab RX: Updated VM broadcast variable value for ID ${variableToRename.id} to "${renameData.newName}"`);
                                }
                                    ...
                                    for (const block of Object.values(blockContainer._blocks)) {
                                        ...
                                        if (broadcastOption && broadcastOption.id === variableToRename.id) {
                                            if (broadcastOption.value !== renameData.newName) {
                                                ...
                                                if (constants.debugging) CollaborationConsole.log(`Collab RX: Updated VM block's BROADCAST_OPTION value for block ${block.id} to "${renameData.newName}"`);
                                            }
                                        }
                                    }
                                }
                                ...
                                resetVMCaches();
                                if (constants.debugging) CollaborationConsole.log("Collab RX: Reset VM caches after broadcast rename.");
                            }
                            if (constants.debugging) CollaborationConsole.log(`Collab RX: Blockly variable renamed for ID ${blocklyVariable.id_} to "${renameData.newName}"`);
                        } catch (e) {
                            CollaborationConsole.error(`Collab RX: Error renaming Blockly variable for ID ${blocklyVariable.id_}:`, e);
                        }
                    }
                }
            } else {
                CollaborationConsole.warn(`Collab RX [var_rename]: Variable "${renameData.oldName}" not found in context "${senderContextTargetName || 'intended global'}" nor globally.`);
            }
        }
        // --- Handle Variable Deletion ---
            ...
            if (!deleteData.ambiguityFix || typeof deleteData.ambiguityFix.varType === "undefined" || typeof deleteData.ambiguityFix.isCloud === "undefined" || typeof deleteData.ambiguityFix.isLocal === "undefined") {
                CollaborationConsole.warn(`Collab RX: var_delete called without ambiguityFix, which is required for correct variable type resolution. Skipping.`);
                return;
            }
                ...
                if (vmTargetForAction) {
                    ...
                    if (vmTargetForAction.variables && vmTargetForAction.variables[variableToDelete.id]) {
                        try {
                            ...
                            if (constants.debugging) CollaborationConsole.log(`Collab RX: Deleted variable "${deleteData.varName}" from ${vmTargetForAction.getName()}`);
                        } catch (e) {
                            CollaborationConsole.error(`Collab RX: Error deleting variable from ${vmTargetForAction.getName()}:`, e);
                        }
                    } else if (constants.debugging) {
                        CollaborationConsole.log(`Collab RX: Variable "${deleteData.varName}" (ID: ${variableToDelete.id}) not found on target ${vmTargetForAction.getName()} for deletion, might have been already deleted.`);
                    }
                    ...
                    if (blocklyVariable) {
                        ...
                        try {
                            ...
                            if (constants.debugging) CollaborationConsole.log(`Collab RX: Blockly variable force-deleted for ID ${blocklyVariable.id_}`);
                        } catch (e) {
                            CollaborationConsole.error(`Collab RX: Error force-deleting Blockly variable for ID ${blocklyVariable.id_}:`, e);
                        }
                    }
                }
            } else {
                CollaborationConsole.warn(`Collab RX [var_delete]: Variable "${deleteData.varName}" not found in context "${senderContextTargetName || 'intended global'}" nor globally.`);
            }
        }
    ...
    if (item.type === constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE) {
        if (!item.data) {
            CollaborationConsole.warn(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Missing data in event item:`, item);
            return;
        }
        const { blocksData, destinationTargetName, sourceTargetName } = item.data;

        if (!blocksData || !destinationTargetName) {
            CollaborationConsole.warn(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Invalid or incomplete data received:`, item.data);
            return;
        }
        ...
        if (!localDestinationTargetId) {
            CollaborationConsole.warn(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Destination target "${destinationTargetName}" not found locally. Skipping VM call.`);
            return;
        }
        ...
        if (sourceTargetName) {
            ...
            if (sourceStage && sourceStage.getName() === sourceTargetName) {
                ...
                if (sourceSprite) {
                    ...
                    CollaborationConsole.warn(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Source target "${sourceTargetName}" not found locally. Proceeding without source ID if applicable.`);
                }
            }
        }

        if (constants.debugging) {
            CollaborationConsole.log(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Attempting to call vm.shareBlocksToTarget with emit=false.`);
            CollaborationConsole.log(`  Destination: "${destinationTargetName}" (Resolved ID: ${localDestinationTargetId})`);
            CollaborationConsole.log(`  Source: "${sourceTargetName || 'N/A'}" (Resolved ID: ${localSourceTargetId || 'N/A'})`);
            CollaborationConsole.log(`  Blocks data (first block ID if exists):`, blocksData?.[0]?.id);
        }
        ...
        try {
            ...
            constants.mutableRefs.vm.shareBlocksToTarget(blocksData, localDestinationTargetId, localSourceTargetId, false)
                .then(() => {
                    if (constants.debugging) CollaborationConsole.log(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: vm.shareBlocksToTarget promise resolved for "${destinationTargetName}".`);
                ...
                .catch(e => {
                    CollaborationConsole.error(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Error during vm.shareBlocksToTarget promise for "${destinationTargetName}":`, e);
                });
        } catch (e) {
            // Catch any synchronous errors that might occur during the initial call.
            CollaborationConsole.error(`Collab RX [${constants.CUSTOM_REMOTE_SHARE_BLOCKS_CALL_TYPE}]: Synchronous error calling vm.shareBlocksToTarget for "${destinationTargetName}":`, e);
        }
    ...
    if (item.event && item.event.type) {
        ...
        if (!workspace) {
            CollaborationConsole.warn(`Collab RX: Workspace not found, cannot apply standard remote event type "${eventJson.type}". Skipping.`);
            return;
        }
        ...
        if (!target) {
            CollaborationConsole.error(`Collab RX: Skipping event type "${eventJson.type}" due to inability to resolve target (Remote: "${remoteTargetName}").`);
            return;
        }
        ...
        // --- START: CUSTOM BLOCK_MOVE HANDLER (FIX for circular dependency & Desync) ---
        if (eventType === constants.mutableRefs.BlocklyInstance.Events.BLOCK_MOVE) {
            ...
            if (!blockToMove) {
                CollaborationConsole.warn(`Collab RX [move]: Block ${eventData.blockId} not found. Skipping move.`);
                return;
            }

            // 1. Unplug the block first.
            if (blockToMove.getParent()) {
                blockToMove.unplug(false);
            }

            let uiConnectionSuccessful = false;
            ...
            if (eventData.newParentId) {
                ...
                let isValid = true;
                if (!newParentBlock) {
                    CollaborationConsole.warn(`Collab RX [move]: New parent block ${eventData.newParentId} not found.`);
                    isValid = false;
                } else {
                    let checkBlock = newParentBlock;
                    while (checkBlock) {
                        if (checkBlock.id === blockToMove.id) {
                            CollaborationConsole.error(`Collab RX [move]: Circular dependency detected.`);
                            isValid = false;
                            break;
                        }
                        checkBlock = checkBlock.getParent();
                    }
                }
                ...
                if (isValid) {
                    ...
                    if (parentConnection && childConnection && parentConnection.checkType_(childConnection)) {
                        ...
                        uiConnectionSuccessful = true; // Success!
                    } else {
                        CollaborationConsole.warn(`Collab RX [move]: Connection invalid or types do not match.`);
                    }
                }
            } 
            // Case B: Moving to a top-level coordinate.
                ...
                // Moving to coordinates is always considered "successful" for VM sync
                uiConnectionSuccessful = true;
            }

            // 3. Manually update the VM's block model.
            if (target.blocks && typeof target.blocks.blocklyListen === 'function') {
                let eventToProcess = eventJson;

                // Instead, tell the VM the block moved to its current unplugged coordinates.
                if (eventData.newParentId && !uiConnectionSuccessful) {
                    if (constants.debugging) {
                        CollaborationConsole.warn(`Collab RX [move]: UI Connection failed. Fallback to coordinate move to prevent VM desync.`);
                    }
                    
                    // Get where the block ended up visually (floating)
                    const currentPos = blockToMove.getRelativeToSurfaceXY();
                    
                    // Create a fallback JSON that removes parent info and sets coordinates
                    // We clone eventJson to avoid mutating the original reference potentially used elsewhere (though unlikely here)
                    const fallbackJson = Object.assign({}, eventJson);
                    delete fallbackJson.newParentId;
                    delete fallbackJson.newInputName;
                    fallbackJson.newCoordinate = `${currentPos.x},${currentPos.y}`;
                    
                    eventToProcess = fallbackJson;
                }

                const blocklyEvent = constants.mutableRefs.BlocklyInstance.Events.fromJson(eventToProcess, workspace);
        ...
        try {
            ...
            CollaborationConsole.error(`Collab RX: Error deserializing standard event JSON type "${eventJson.type}":`, e, eventJson);
            return;
        }
        
        try {
            if (constants.debugging) {
                CollaborationConsole.log(`Collab RX: Processing standard event=${blocklyEvent.type} (Block ID: ${blocklyEvent.blockId || 'N/A'})`);
                CollaborationConsole.log(`  VM Target for event: "${target.getName()}" (Origin Context: "${remoteTargetName || 'N/A'}")`);
                CollaborationConsole.log(`  Current local view: "${currentEditingTargetName || 'N/A'}"`);
            }
            ...
            if (isViewingCorrectTarget || isPotentiallyGlobalEvent || constants.mutableRefs.BlocklyInstance.Events.VAR_CREATE === blocklyEvent.type) {
                ...
                if (blocklyEvent.type === constants.mutableRefs.BlocklyInstance.Events.BLOCK_CREATE && blocklyEvent.blockId && currentWorkspace.getBlockById(blocklyEvent.blockId)) {
                    if (constants.debugging) CollaborationConsole.log(`Collab RX: Skipping visual .run() for standard CREATE on block ${blocklyEvent.blockId} - block already exists in current workspace view.`);
                    ...
                    if (!currentWorkspace.getBlockById(blocklyEvent.blockId)) {
                        if (constants.debugging) CollaborationConsole.log(`Collab RX: Skipping visual .run() for ${blocklyEvent.type} on block ${blocklyEvent.blockId} - block not found in current workspace view.`);
                        canRunVisually = false;
                    }
                }

                if (canRunVisually) {
                    if (constants.debugging) CollaborationConsole.log(`Collab RX: Applying standard event ${blocklyEvent.type} VISUALLY.`, item);
                    ...
                    if (item?.event?.xml?.includes("procedures_definition") || item?.event?.newValue?.includes("argumentids") || blocklyEvent.type === "delete") {
                        CollaborationConsole.log(`Collab RX: Refreshing toolbox`);
                        constants.mutableRefs.BlocklyInstance.getMainWorkspace().refreshToolboxSelection_();
                    }
                }
            } else if (constants.debugging) CollaborationConsole.log(`Collab RX: Skipping visual application of standard event ${blocklyEvent.type} because remote target "${remoteTargetName}" is not the current view "${currentEditingTargetName}".`);
            ...
            if (target.blocks && typeof target.blocks.blocklyListen === 'function') {
                ...
                if (blocklyEvent.type === constants.mutableRefs.BlocklyInstance.Events.BLOCK_CREATE && blocklyEvent.blockId && target.blocks.getBlock(blocklyEvent.blockId)) {
                    if (constants.debugging) CollaborationConsole.log(`Collab RX: Skipping VM .blocklyListen() for standard CREATE on block ${blocklyEvent.blockId} - block already exists in VM target "${target.getName()}".`);
                    canRunVmUpdate = false;
                }

                if (canRunVmUpdate) {
                    if (constants.debugging) CollaborationConsole.log(`Collab RX: Updating VM state for Target="${target.getName()}" via blocklyListen for event type ${blocklyEvent.type}.`);
                    target.blocks.blocklyListen(blocklyEvent);
                }
            } else {
                CollaborationConsole.error(`Collab RX: Failed to update VM state for standard event type ${blocklyEvent?.type}. Target "${target?.getName()}" or its 'blocks.blocklyListen' method not found.`);
            }

        } catch (e) {
            CollaborationConsole.error(`Collab RX: Error applying remote standard event type ${blocklyEvent?.type} (VM Target: ${target?.getName()}, Block ID: ${blocklyEvent?.blockId}):`, e, blocklyEvent);
        }
    }
    ...
    // --- Handle unhandled custom event types ---
        ...
        CollaborationConsole.warn('Collab RX: Received unhandled custom event item type:', item.type, item);
    }
    // --- Handle malformed event items ---
        ...
        CollaborationConsole.warn('Collab RX: Received malformed event item with no type and no event field:', item);
    }
}
